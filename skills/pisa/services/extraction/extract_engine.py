"""
PISA V2 — Slide-to-Canvas Extraction Engine
...
def build_canvas_v2(classification, shapes, roles, groups, layout,
...
def register_canvas_v2(prim, lib_path="pisa_library.json", mode="add"):
    if os.path.exists(lib_path):
        with open(lib_path) as f: lib = json.load(f)
    else:
        lib = {"schema_version":2,"canvases":[],"meta":{"version":1,"updated":None}}

    # Same-source versioning
    for ex in lib["canvases"]:
        ...
        if sa.get("file")==sb.get("file") and sa.get("slide")==sb.get("slide"):
            ...
            lib["canvases"].remove(ex)
            lib["canvases"].append(prim)
...
def generate_svg_preview(prim, theme):
    """Generate an SVG preview string for a canvas. Pure Python, no external deps.
    ...
    try:
        ...
        return {"svg": None, "note": "SVG renderer not available — install services/svg/canvas_to_svg.py"}
...
def extract_deck_v2(pptx_path, theme=None, lib_path="pisa_library.json", generate_previews=True):
    ...
    for i, slide in enumerate(prs.slides):
        ...
        prim = build_canvas_v2(cls, content, roles, groups, layout, tokens, order, i+1, src)
        reg = register_canvas_v2(prim, lib_path)
        svg = None
        if generate_previews and theme and reg["action"] in ("added","versioned"):
            ...
            # Store SVG in the canvas within the library
            if svg:
                prim["svg"] = svg
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="PISA V2 Slide-to-Canvas Extractor")
