...
def render_svg(canvas, theme=None, width=960, height=540, show_labels=True, show_groups=True):
    if theme is None: theme = NEUTRAL_THEME
    components = canvas.get("components", [])
    groups = canvas.get("semantic_groups", [])
    intent = canvas.get("intent", "unknown")
    layout = canvas.get("layout_type", "")
    lv = canvas.get("layout_variant", "")
    quality = canvas.get("quality_score", 0)
    accent = _t(theme, "token.color.accent")
...
def render_svg_grid(canvases, theme=None, columns=3, cell_w=320, cell_h=180, gap=12):
    if theme is None: theme = NEUTRAL_THEME
    n = len(canvases); rows = math.ceil(n / columns)
    tw = columns*cell_w+(columns-1)*gap; th = rows*(cell_h+22)+(rows-1)*gap
    parts = [f'<svg viewBox="0 0 {tw} {th}" xmlns="http://www.w3.org/2000/svg">']
    for i, prim in enumerate(canvases):
...
def render_svg_theme_compare(canvas, themes_dict, cell_w=440, cell_h=250, gap=20):
    n = len(themes_dict); tw = n*cell_w+(n-1)*gap; th = cell_h+24
    parts = [f'<svg viewBox="0 0 {tw} {th}" xmlns="http://www.w3.org/2000/svg">']
    for i, (name, theme) in enumerate(themes_dict.items()):
        ox = i*(cell_w+gap)
        inner = render_svg(canvas, theme, cell_w, cell_h, False, False)
        content = inner.split(">",1)[1].rsplit("</svg>",1)[0]
