...
def render_svg(primitive, theme=None, width=960, height=540, show_labels=True, show_groups=True):
    if theme is None: theme = NEUTRAL_THEME
    components = primitive.get("components", [])
    groups = primitive.get("semantic_groups", [])
    intent = primitive.get("intent", "unknown")
    layout = primitive.get("layout_type", "")
    lv = primitive.get("layout_variant", "")
    quality = primitive.get("quality_score", 0)
    accent = _t(theme, "token.color.accent")
...
def render_svg_grid(primitives, theme=None, columns=3, cell_w=320, cell_h=180, gap=12):
    if theme is None: theme = NEUTRAL_THEME
    n = len(primitives); rows = math.ceil(n / columns)
...
def render_svg_theme_compare(primitive, themes_dict, cell_w=440, cell_h=250, gap=20):
    n = len(themes_dict); tw = n*cell_w+(n-1)*gap; th = cell_h+24
    parts = [f'<svg viewBox="0 0 {tw} {th}" xmlns="http://www.w3.org/2000/svg">']
    for i, (name, theme) in enumerate(themes_dict.items()):
        ox = i*(cell_w+gap)
        inner = render_svg(primitive, theme, cell_w, cell_h, False, False)
        content = inner.split(">",1)[1].rsplit("</svg>",1)[0]
...
def generate_pack_svgs(pack, theme_name=None):
    ...
    for prim in pack.get("primitives", []):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--primitive", help="Path to primitive JSON")
    parser.add_argument("--theme", help="Path to theme JSON")
    parser.add_argument("--output", default="output.svg")
    args = parser.parse_args()
    if args.primitive:
        with open(args.primitive) as f: prim = json.load(f)
