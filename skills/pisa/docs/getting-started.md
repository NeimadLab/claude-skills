...
PISA turns Claude into a presentation specialist. Upload any deck — PPTX, PDF, or screenshot — and Claude reverse-engineers every slide into a reusable **canvas**: a JSON object that captures structure, semantics, and visual treatment without any rendering decisions.

Generate new decks by selecting canvases, filling them with content, and rendering to PPTX. Retheme an entire presentation by swapping one JSON file. Review decks against a professional scoring rubric.
...
→ 24 canvases across 21 intents, loaded in seconds.
...
```
Upload this PDF and extract all slide canvases
```
...
| Capability | Trigger |
|-----------|---------|
| **Analyze** | Upload PPTX/PDF/image → decompose into canvases |
| **Generate** | "Create a deck about..." → select canvases → render PPTX |
...
| **Local override** | Upload your deck → your canvases take priority over registry |
