# PISA Canvas Versioning & Diff

## Purpose

Track canvas evolution over time. When a deck is re-extracted after modifications,
...
Same-source detection: if a newly extracted canvas has the same `source.file` +
...
// pisa_version_log.json
{
  ...
  "entries": [
    {
      "canvas_id": "prim_quarterly_3_a8f2c1d9e4b7",
      "from_version": 1,
      "to_version": 2,
...
def diff_canvases(prim_a, prim_b):
    """Compare two canvas versions. Returns structured diff report."""
    diff = {
...
```bash
# Show diff between two versions (by canvas ID)
python3 scripts/library/diff.py --id prim_deck_3_a8f2 --v1 1 --v2 2

# Show version history for a canvas
