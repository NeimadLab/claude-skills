...
A PISA registry is a GitHub repository that serves as the distribution hub for slide canvases, themes, and personas. Claude fetches content via raw URLs. No server, no database — just JSON files in a Git repo.
  ...
  "packs": [
    {
      ...
      "canvases": 24, "intents_covered": 21, "quality_min": 7.0,
...
→ 24 canvases loaded. Ready to generate.
```
...
**Adding a pack:** Extract canvases from reference deck → ask Claude to export as pack JSON → validate (schema 2.1, quality ≥7.0, kpi{} on KPI cards, visual{} on all components, embedded SVGs) → add to packs/ → update registry.json → open PR.
...
```yaml
title: PISA Registry
description: Slide canvas packs, themes, and personas
theme: minima
...
Fields: pack name, target audience, estimated canvases, intents needed (checklist), reference material available?
...
1. Upload PPTX to Claude → "Extract all slides as PISA canvases"
...
### From Scratch
Ask Claude: "Create a PISA pack with 10 canvases for [use case]"
...
Semantic versioning: 1.0 → initial, 1.1 → new canvases, 2.0 → breaking schema change. Update version in both the content file and registry.json. Claude detects version changes on "check for updates".
