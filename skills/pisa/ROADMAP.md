...
The pipeline works end-to-end: templates → theme resolution → PPTX generation → QA. 61 templates across 4 packs, enriched with dark/light alternation, icons, KPI labels, and trend indicators. The registry is live, the generator produces real PPTX files.
...
- [x] 61 templates across 4 packs (Corporate, Finance, Strategy, Startup)
...
- [x] PPTX generator (`generate.js`): template + theme + content → real PPTX
- [x] All 61 templates enriched: dark/light/accent cards, 61 icons, 31 KPI labels
- [x] 15 layout variants (was 1)
- [x] Built-in registry URL — zero user configuration
- [x] Template resolution order: local > override > registry
- [x] `source.origin` field on all templates
...
### Hero templates (top 10 redesign)
...
Right now themes swap colors. They should transform the template feel:
...
- [ ] Same template + different theme = visually distinct result (not just recolored)

### Persona shaping beyond density
Personas should transform template selection and content, not just cap word counts:
...
- [ ] Persona influences which templates are eligible: `executive` never selects `funnel` or `org_chart`
...
### Typed placeholders
Templates should have typed slots, not just `text_preview` strings:
...
## 🔬 v1.4 — Extract & Learn
**Goal:** Upload any deck → extract templates that are actually reusable.
**Target:** June 2026
...
**Goal:** Other people contribute templates, packs, and themes.
**Target:** July 2026

### Contribution system
- [ ] CI validates submitted templates (schema compliance + SVG preview)
...
### Auto-layout
- [ ] Content → auto-select best template (no user choice needed)
- [ ] "Reflow this slide" → show 3 alternative layouts as SVG
- [ ] Content-aware: long text → stacked, many numbers → grid

### Brand intelligence
- [ ] Upload brand guidelines PDF → auto-generate theme + templates
...
The highest-impact work right now is in **v1.2** — making slides beautiful. If you can design a better version of any template, that's the most valuable contribution.
