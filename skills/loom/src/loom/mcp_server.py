...
TOOLS = [
    ...
    Tool(
        name="loom_open_session",
        description=(
            "Open a session to track which AI tool is working. "
            "Call at the start of every interaction."
        ),
        inputSchema={
            "type": "object",
            "properties": {
                "actor": {
                    "type": "string",
                    "description": "Tool name: claude-code, cursor, windsurf, etc.",
                },
                "model_name": {
                    "type": "string",
                    "description": "Model name: claude-opus-4, gpt-4o, etc.",
                },
            },
            "required": ["actor"],
        },
    ...
    Tool(
        name="loom_close_session",
        description="Close the current session. Call when finishing work.",
        inputSchema={
            "type": "object",
            "properties": {
                "summary": {
...
def handle_get_state(workspace: Path) -> dict:
    """Handle loom_get_state tool call."""
    return get_workspace_state(workspace)


def handle_open_session(arguments: dict, workspace: Path) -> dict:
    """Handle loom_open_session tool call."""
...
def handle_close_session(arguments: dict, workspace: Path) -> dict:
    """Handle loom_close_session tool call."""
    from loom.sessions import SessionStore
    ...
    result = store.close_session(active["id"], arguments.get("summary"))
    store.close()
    return result
...
def create_server(workspace: Path | None = None) -> Server:
    ...
    async def call_tool(name: str, arguments: dict) -> list[TextContent]:
        ...
        try:
            ...
            elif name == "loom_open_session":
                result = handle_open_session(arguments, ws)
            elif name == "loom_close_session":
                result = handle_close_session(arguments, ws)
            else:
